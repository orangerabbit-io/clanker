...
class SseReader(private val readLine: suspend () -> String?) {
     ...
     *
     * Raw capture: the verbatim `data:` payload of every successfully parsed
     * chunk is accumulated (before the [DONE] sentinel) and returned alongside
     * the drain result so callers can persist it (wire-faithful restarts).
     */
    suspend fun events(
